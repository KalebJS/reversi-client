...
class GameNode:
        ...
        for move in valid_moves:
            ...
            if depth > 1:
                if (self.winning_child == None):
                    ...
                    child.generate_child_states(depth - 1, maximizer, minimizer, max(self.winning_child.get_score(maximizer), alpha), min(self.winning_child.get_score(maximizer), beta))
    ...
    def generate_state_for_move(self, current_state, move_to_make):
        ...
        def capture(self, board, row, col, xdir, ydir, could_capture=0):
            ...
            return capture(self, board, row + ydir,
                                        col + xdir,
                                        xdir, ydir,
                                        could_capture + 1)
        ...
        for xdir in range(-1, 2):
            for ydir in range(-1, 2):
                ...
                success, tmp_board = capture(self, tmp_board, move_to_make[0] + ydir, move_to_make[1] + xdir, xdir, ydir)
                if success:
                    new_board = tmp_board
